...
fn run_complete_command(
    ...
    let _ = pipe.set(-1, unistd::getpgrp(), core);
...
pub(super) fn command_names(core: &ShellCore) -> Vec<Vec<u8>> {
...
pub(super) fn variable_names(core: &mut ShellCore) -> Vec<Vec<u8>> {
...
mod tests {
    ...
    fn command_names_include_keywords_for_readline_commands() {
        ...
        assert!(names.iter().any(|name| name == b"if"));
        assert!(names.iter().any(|name| name == b"while"));
    }
}
